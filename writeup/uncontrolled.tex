...
Early-outbreak stochasticity can impact when an epidemic becomes established and begins its exponential growth phase \citep{morris_computation_2024}. This time shift durably impacts the epidemic's full trajectory. The effect of this early-outbreak stochasticity can be modeled as a random time shift applied to the deterministic epidemic trajectory \citep{morris_computation_2024}. Here, we demonstrate that the distribution of this time shift --- both its mode and variance --- depend on $\psi$. 
...
\end{equation}
%
Here, $r$ is the early-outbreak exponential growth rate and $E[W]$ is the initial number of infectious individuals, $I_0$. For clarity of the derivation, we use $E[W]=I_0=1$ throughout. 

The random scaling factor $W$ can, through a change of variables, be expressed as a random time shift: 
%
\begin{equation}
...
Given this setup, it can be shown ({\bf Supplementary Methods}) that
%
\begin{equation}
	\text{Var}[W] = \frac{
...
\end{equation}
%
where $z = R_0^{1/\alpha} - 1$. The punctuation parameter $\psi$ enters the expression only once, in the middle term of the numerator; thus, $\text{Var}[W]$ and $\psi$ are monotonically related, where narrower individual infectiousness profiles ($\psi \rightarrow 0$) maximize the variance and broader profiles ($\psi \rightarrow 1$) minimize it. 
...
\end{equation}
%
provided the numerator within the logarithm is positive; when the numerator is zero or negative, no finite modal time shift exists and  $\text{Mode}[\varsigma]  \rightarrow -\infty$. Again, $\psi$ enters the expression only once, with narrow individual infectiousness profiles ($\psi \rightarrow 0$) minimizing the mode (making it more negative, \ie, causing the typical epidemic to be shifted later) and broad individual infectiousness profiles ($\psi \rightarrow 1$) maximizing it (causing the typical epidemic to be shifted earlier). 
