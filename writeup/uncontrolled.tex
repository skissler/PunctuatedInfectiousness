...
\end{equation}
%
since, with a single index case, $\log E[W] = \log 1 = 0$. 

Under the Gamma time-shift model for infectiousness profiles, It can be shown ({\bf Supplementary Methods}) that
%
\begin{equation}
	\text{Var}[W] = \frac{
...
We can also derive an approximate expression for the mode of the distribution of $\varsigma$. Using a two-moment version of the moment matching approach of \citet{morris_computation_2024}, we can approximate the distribution of the time shift $\varsigma$. For tractability, we use only the first two moments here. It can be shown ({\bf Supplementary Methods}) that
%
\begin{equation}
