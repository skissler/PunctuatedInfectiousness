...
Let  contacts be uniform over time and across individuals ($c_i(t) \equiv 1 \, \forall \, i$), and let $b_i(\tau) $ follow the time-shifted Gamma model (Eq.~\ref{eq:gamma_timeshift}). Drawing upon branching process theory, the cumulative incidence over time, $Z(t)$, during the exponential growth phase can be approximated as a deterministic exponential function with a random initial scaling factor $W$: 
%
\begin{equation}
...
We can also derive an approximate expression for the mode of the distribution of $\varsigma$, $f_\varsigma(\tau)$. To approximate $f_\varsigma(\tau)$, we can use  a two-moment version of the moment matching approach of \citet{morris_computation_2024}. These moments are 
%
\begin{equation}
...
Note that the $\psi$-dependence of both $\text{Var}[W]$ and $\text{Mode}[\varsigma]$ enter through the expression 
%
\begin{equation}
